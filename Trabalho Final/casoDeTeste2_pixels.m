...
treinoX = csvread('pixelsTreino28x28X.csv');
treinoy = csvread('pixelsTreino28x28y.csv');
X = treinoX(2:end, :);
y = treinoy(2:end, :);
size(X)

% Carregando dados de teste
testeX = csvread('pixelsTeste28x28X.csv');
testey = csvread('pixelsTeste28x28y.csv');
X_ = testeX(2:end, :);
size(X_)
y_ = testey(2:end, :);
...
for i=1:10
  ...
  accTrain += mean(double(predTrain == y)) * 100;
  
  predTest = predict(Theta1, Theta2, X_);
  accTest += mean(double(predTest == y_)) * 100;
  epoch += 1
endfor
